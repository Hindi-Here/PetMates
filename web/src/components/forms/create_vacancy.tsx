...
import { useBlockScroll, useTagInput, useChangeInput, validatorFormat, validatorRegex  } from '../scripts/function';
...
const Form = ({ onClose }: any) => {
  ...
  const { data, handleChange } = useChangeInput({
    ...
    {
      role: validatorRegex.username,
      message: validatorRegex.email,
      tags: validatorRegex.tags
    });
...
const validate = (data: any): boolean => {
  return !validatorFormat.required(data.role) || 
         ...
         !validatorFormat.required(data.message) ||
         !validatorFormat.maxLength(data.message, 500) ||
         
         !validatorFormat.required(data.tags) ||
         !validatorFormat.hasTag(data.tags)
};
